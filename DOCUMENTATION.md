...
    ├── intraday\_scheduler.py  # Market hours intraday daemon background scheduler

    ├── tasks/                 # Windows Task Scheduler & batch script files

    │   ├── run\_scanner.bat            # Daily market close execution batch script

    │   ├── run\_intraday\_scanner.bat   # Market hours intraday batch script

    │   ├── setup\_task.ps1             # Windows Task Scheduler script for daily scan (4:05 PM)

    │   └── setup\_intraday\_task.ps1    # Windows Task Scheduler script for intraday scan (9:15 AM - 3:30 PM)
...
    │   ├── intraday\_scanner.py # Market hours intraday portfolio scanner
...
    │   ├── report\_generator.py # Markdown digest builder

    │   └── view\_alerts.py     # CLI interactive alert browser & filter
...
    ├── logs/                  # Daily runtime log files (\`india\_swing\_YYYYMMDD.log\`)
...
TELEGRAM\_BOT\_TOKEN=123456789\:ABCdefGHIjklMNOpqrsTUVwxyZ

TELEGRAM\_CHAT\_ID=987654321

\# Alert Thresholds

ALERT\_MIN\_SCORE=70

MIN\_PRICE=50.0

MIN\_AVG\_DAILY\_VOLUME=200000

ALERT\_DEDUP\_HOURS=4
...
3\. Give your bot a name (e.g., \`MySwingAlertBot\`) and a username ending in \`bot\` (e.g., \`MySwingAlerts\_bot\`).

4\. Copy the HTTP API **\*\*Bot Token\*\*** (looks like \`123456789\:AAF...\`).

5\. Paste this as \`TELEGRAM\_BOT\_TOKEN\` in \`bot/.env\`.
...
4\. Paste this as \`TELEGRAM\_CHAT\_ID\` in \`bot/.env\`.
...
2026-08-28 17:15:00 [INFO] [\_\_main\_\_]   telegram: OK ✓
...
  python -m alerts.intraday\_scanner
...
  python -m alerts.intraday\_scanner --force
...
  .\tasks\run\_intraday\_scanner.bat
...
  Registers a Windows Scheduled Task \`SwingBotIntradayScanner\` that executes \`run\_intraday\_scanner.bat\` **\*\*every 15 minutes\*\*** between **\*\*09:15 AM and 03:30 PM IST\*\***, Monday through Friday:

  \`\`\`powershell

  cd "d:\Stock Bot\bot"

  powershell -ExecutionPolicy Bypass -File .\tasks\setup\_intraday\_task.ps1
...
  python intraday\_scheduler.py --interval 15
...
python -m analytics.view\_alerts --intraday
...
A PowerShell script \`bot/tasks/setup\_task.ps1\` automatically registers a background task named \`SwingBotScanner\`.
...
   powershell -ExecutionPolicy Bypass -File .\tasks\setup\_task.ps1
...
This daemon sleeps until 16:05 IST each market weekday, triggers \`scan\_universe()\`, and repeats automatically.
...
**### 1. Interactive Alert Browser (\`analytics.view\_alerts\`)**
...
  python -m analytics.view\_alerts
...
  python -m analytics.view\_alerts --min-score 80
...
  python -m analytics.view\_alerts --symbol RELIANCE
...
  python -m analytics.view\_alerts --date 20260828
...
  python -m analytics.view\_alerts --history
...
 BREAKOUT\_UP        25       68.0%      +3.20%

 REVERSAL\_UP        18       62.5%      +2.40%

 RISING\_PEAKS       12       72.0%      +2.90%

\======================================================================

\`\`\`

**### 3. Markdown Digest Generator (\`analytics.report\_generator\`)**

Creates a formatted markdown digest report stored at \`bot/reports/summary\_report.md\`:

\`\`\`powershell

python -m analytics.report\_generator
...
   - \`BREAKOUT\_UP\`: Price breaks above 20-day high with volume > 1.5× 20-day SMA and RSI > 55.

   - \`REVERSAL\_UP\`: Bullish candlestick pattern at lower Bollinger Band with oversold RSI recovery.

   - \`MACD\_UP\`: MACD line crosses above Signal line with positive histogram slope.

   - \`RISING\_PEAKS\`: Trend confirmation with higher highs and higher lows over 20 candles.

   - \`GAP\_UP\`: Bullish opening gap with strong volume continuation.
...
   - Stores sent alerts in \`bot/alerts/alert\_history.json\`.
...
\| \`Telegram credentials not configured\` | Missing or blank \`.env\` values | Ensure \`TELEGRAM\_BOT\_TOKEN\` and \`TELEGRAM\_CHAT\_ID\` are set in \`bot/.env\`. |
...
\| Scheduled Task didn't run | Windows sleep mode or permission issue | Verify \`SwingBotScanner\` or \`SwingBotIntradayScanner\` in Task Scheduler (\`taskschd.msc\`). Run \`bot/setup\_task.ps1\` or \`bot/setup\_intraday\_task.ps1\` to re-register tasks. |

\| View logs | Check daily logs | Open log files in \`bot/logs/stock\_bot\_YYYYMMDD.log\`. Log timestamps are formatted in IST. |
